...
byte keyState = 0; // key state from port read?
bool keyPressed[5] = {0,0,0,0,0}; //save keyboard button states for debounce
...
{ 
  byte i = 0;
  if (bitRead(keyState, i) == HIGH && keyPressed[i] == LOW)
  { 
    // If button is high (pressed)
    keyPressed[i] = 1;
  }
  
  else if (bitRead(keyState, i) == LOW && keyPressed[i] == HIGH)
  {
    // if button is low (released)  
